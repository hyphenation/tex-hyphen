\directlua{
  function load_patterns(tag, dir)
    ...
    if dir
    then pattdir = dir
    else pattdir = "/Users/arthur/TeX/hyphen/svn/trunk/hyph-utf8/tex/generic/hyph-utf8/patterns/txt"
    end
...
\def\stopcollecthyphens{%
  \par
  \directlua{callback.register("pre_linebreak_filter", nil)}
}
...
\def\dumpnodelist#1{%
  \directlua{callback.register("pre_linebreak_filter", function(head)
    dump_node_list(head)
  end)}
  #1\par
  \directlua{callback.register("pre_linebreak_filter", nil)}
...
\long\def\comparehyphenation#1{%
  ...
  \directlua{
    ws = { }
    hs = { }
    for w in unicode.utf8.gmatch("#1", "\nonspace*") do
      if w and w:len() > 0 then
        table.insert(ws, w)
...
%
  \directlua{
    local n = table.maxn(ws)
    for k = 1, n do
      texio.write_nl("term and log", tostring(ws[k]) .. " (" .. tostring(hs[k]) .. ")")
    end
